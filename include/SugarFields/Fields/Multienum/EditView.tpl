...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
	...
	{if !empty($fields.id.value)}
		{multienum_to_array string={{sugarvar key='value' string=true}} assign="values"}
	{else}
		{multienum_to_array string={{sugarvar key='value' string=true}} default={{sugarvar key='default' string=true}} assign="values"}
	{/if}	<select id="{{if empty($displayParams.idName)}}{{sugarvar key='name'}}{{else}}{{$displayParams.idName}}{{/if}}"
